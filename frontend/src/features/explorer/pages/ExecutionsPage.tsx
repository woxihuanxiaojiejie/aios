...
import type { ExecutionListItem, ExplorerFilters } from "../types";
...
export function ExecutionsPage() {
  ...
  const list = useQuery({
    queryKey: ["simulated-executions", current, filters],
    queryFn: () =>
      dataProvider().getList<ExecutionListItem>({
        resource: "simulated-executions",
  ...
  const columns: ColumnsType<ExecutionListItem> = [
    ...
    { title: "Side / Action", dataIndex: "action", width: 130 },
    { title: "Quantity", dataIndex: "quantity", width: 110 },
    { title: "Simulated Price", dataIndex: "simulated_price", width: 140 },
    {
      title: "Status",
      dataIndex: "status",
      width: 140,
    ...
    },
    {
      ...
      width: 170,
      render: (value: string | null) =>
        value ? <Link to={`/settlements/${encodeURIComponent(value)}`}>{value}</Link> : "等待结算",
    },
    {
      title: "Action",
      key: "action",
      ...
      render: (_value, record) => (
        <Button
          ...
          onClick={(event) => {
            ...
            navigate(`/executions/${encodeURIComponent(record.execution_id)}`);
          }}
        >
          Detail
        </Button>
  ...
  return (
    <section className="research-page">
      <div className="page-heading">
        <Typography.Title level={2}>Simulated Executions</Typography.Title>
      </div>
      ...
      <Table
        ...
        locale={{ emptyText: <EmptyState description="暂无 Simulated Execution" /> }}
        pagination={{
          current,
          pageSize: PAGE_SIZE,
        ...
        onChange={changePage}
        onRow={(record) => ({
          onClick: () => navigate(`/executions/${encodeURIComponent(record.execution_id)}`),
        })}
      />
    </section>
  );
